...
async function apagarMensagemUsuario(env, message) { if (!message?.chat?.id || !message?.message_id) return; try { await enviarTelegram(env.TELEGRAM_TOKEN, "deleteMessage", { chat_id: message.chat.id, message_id: message.message_id }); } catch {} }
...
async function oferecerLegendaOuBotoes(env, chatId, state) {
  const key=`state_${state.userId}`;
  if (midiaAceitaLegenda(state.mediaType) && !temLegenda(state)) {
    state.step="WAITING_CAPTION_CHOICE"; await env.KV_BOT_BANNERS.put(key,JSON.stringify(state),{expirationTtl:3600});
    return atualizarPainel(env,chatId,state.panelMessageId,"📝 <b>MÍDIA RECEBIDA SEM LEGENDA</b>\n\nDeseja adicionar uma legenda antes de continuar?",[[{text:"📝 Adicionar Legenda",callback_data:"disparo:legenda"}],[{text:"➡️ Continuar sem Legenda",callback_data:"disparo:sem_legenda"}],[{text:"❌ Cancelar",callback_data:"disparo:cancelar"}]]);
  }
  return oferecerBotoes(env,chatId,state);
}
...
async function processarMidia(env,m,state){ const userId=m.from.id.toString(),key=`state_${userId}`;
  if(m.photo?.length){state.mediaType="photo";state.fileId=m.photo.at(-1).file_id;state.caption=m.caption||""}else if(m.video){state.mediaType="video";state.fileId=m.video.file_id;state.caption=m.caption||""}else if(m.animation){state.mediaType="animation";state.fileId=m.animation.file_id;state.caption=m.caption||""}else if(m.document){state.mediaType="document";state.fileId=m.document.file_id;state.caption=m.caption||""}else if(m.audio){state.mediaType="audio";state.fileId=m.audio.file_id;state.caption=m.caption||""}else if(m.voice){state.mediaType="voice";state.fileId=m.voice.file_id;state.caption=m.caption||""}else if(m.video_note){state.mediaType="video_note";state.fileId=m.video_note.file_id;state.caption=""}else if(m.sticker){state.mediaType="sticker";state.fileId=m.sticker.file_id;state.caption=""}else if(m.poll){state.mediaType="poll";state.poll={question:m.poll.question,options:m.poll.options.map(o=>o.text),is_anonymous:m.poll.is_anonymous,type:m.poll.type,allows_multiple_answers:m.poll.allows_multiple_answers,correct_option_id:m.poll.correct_option_id,explanation:m.poll.explanation||undefined};state.caption=""}else if(m.location){state.mediaType="location";state.location={latitude:m.location.latitude,longitude:m.location.longitude};state.caption=""}else if(m.venue){state.mediaType="venue";state.venue={latitude:m.venue.location.latitude,longitude:m.venue.location.longitude,title:m.venue.title,address:m.venue.address};state.caption=""}else if(m.contact){state.mediaType="contact";state.contact={phone_number:m.contact.phone_number,first_name:m.contact.first_name,last_name:m.contact.last_name||undefined,vcard:m.contact.vcard||undefined};state.caption=""}else if(m.dice){state.mediaType="dice";state.diceEmoji=m.dice.emoji;state.caption=""}else if(m.text){state.mediaType="text";state.fileId=null;state.caption=m.text}else return;
  await apagarMensagemUsuario(env,m);state.userId=userId;state.buttons=null;await env.KV_BOT_BANNERS.put(key,JSON.stringify(state),{expirationTtl:3600}); const direto=["poll","sticker","video_note","location","venue","contact","dice"];if(direto.includes(state.mediaType)){state.step="WAITING_CONFIRMATION";await env.KV_BOT_BANNERS.put(key,JSON.stringify(state),{expirationTtl:3600});return mostrarConfirmacao(env,m.chat.id,state)} return oferecerLegendaOuBotoes(env,m.chat.id,state);
}
