...
async function processarBotoes(env,message,state){const userId=message.from.id.toString();if(!message.text)return;try{state.buttons=parseButtons(message.text)}catch(e){await apagarMensagemUsuario(env,message);return atualizarPainel(env,message.chat.id,state.panelMessageId,`⚠️ <b>Formato de botão inválido.</b>\n\n${escapeHtml(e.message)}`,[[{text:"❌ Cancelar",callback_data:"disparo:cancelar"}]])}await apagarMensagemUsuario(env,message);state.step="WAITING_PIN_CHOICE";state.panelMessageId=null;await env.KV_BOT_BANNERS.put(`state_${userId}`,JSON.stringify(state),{expirationTtl:3600});return enviarTelegram(env.TELEGRAM_TOKEN,"sendMessage",{chat_id:message.chat.id,text:"📌 <b>FIXAR PUBLICAÇÃO</b>\n\nDeseja fixar esta postagem nos grupos após o envio?",parse_mode:"HTML",reply_markup:{inline_keyboard:[[{text:"📌 Sim, fixar",callback_data:"disparo:fixar"}],[{text:"➡️ Não fixar",callback_data:"disparo:nao_fixar"}],[{text:"❌ Cancelar",callback_data:"disparo:cancelar"}]]}})}
