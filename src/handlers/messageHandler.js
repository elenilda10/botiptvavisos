...
export async function handleMessage(env, message) {
  ...
  switch (state.step) {
    ...
    case "WAITING_ALBUM": return processarAlbum(env, message, state);
...
function itemAlbum(message) {
  if (message.photo?.length) return { type: "photo", media: message.photo[message.photo.length - 1].file_id, caption: message.caption || "" };
  if (message.video) return { type: "video", media: message.video.file_id, caption: message.caption || "" };
  if (message.document) return { type: "document", media: message.document.file_id, caption: message.caption || "" };
  if (message.audio) return { type: "audio", media: message.audio.file_id, caption: message.caption || "" };
  return null;
}

async function processarAlbum(env, message, state) {
  const userId = message.from.id.toString();
  const stateKey = `state_${userId}`;
  if (!message.media_group_id || message.media_group_id !== state.mediaGroupId) return;
  const item = itemAlbum(message);
  if (!item) return;

  state.album ||= [];
  if (!state.album.some(x => x.messageId === message.message_id)) state.album.push({ ...item, messageId: message.message_id });
  state.album.sort((a, b) => a.messageId - b.messageId);
  ...
  // Cada item do álbum chega como update separado. Pequena espera permite juntar o lote.
  await new Promise(resolve => setTimeout(resolve, 1200));
  const atualRaw = await env.KV_BOT_BANNERS.get(stateKey);
  if (!atualRaw) return;
  const atual = JSON.parse(atualRaw);
  if (atual.step !== "WAITING_ALBUM" || Date.now() - (atual.albumLastAt || 0) < 900) return;

  atual.mediaType = "album";
  atual.buttons = null;
  atual.step = "WAITING_BUTTON_CHOICE";
  await env.KV_BOT_BANNERS.put(stateKey, JSON.stringify(atual), { expirationTtl: 3600 });
  return atualizarPainel(env, message.chat.id, atual.panelMessageId,
    `✅ <b>ÁLBUM RECEBIDO</b>\n\n🖼 Itens: <b>${atual.album?.length || 0}</b>\n\nDeseja adicionar botões após o álbum?`,
...
async function processarMidia(env, message, state) {
  ...
  if (message.media_group_id) {
    ...
    state.album = [{ ...item, messageId: message.message_id }];
    state.albumLastAt = Date.now();
    state.buttons = null;
...
async function processarBotoes(env, message, state) {
  ...
  catch (error) { await apagarMensagemUsuario(env, message); return atualizarPainel(env, message.chat.id, state.panelMessageId, `⚠️ <b>Formato de botão inválido.</b>\n\n${escapeHtml(error.message)}\n\nUse:\n<code>Comprar - https://site.com</code>`, [[{ text: "❌ Cancelar", callback_data: "disparo:cancelar" }]]); }
  await apagarMensagemUsuario(env, message);
  state.userId = userId;
  state.step = "WAITING_CONFIRMATION";
  await env.KV_BOT_BANNERS.put(stateKey, JSON.stringify(state), { expirationTtl: 3600 });
...
async function processarGrupo(env, message, state) {
  ...
  await apagarMensagemUsuario(env, message); await adicionarGrupo(env, id); await env.KV_BOT_BANNERS.delete(`state_${message.from.id}`);
  return atualizarPainel(env, message.chat.id, state.panelMessageId, `✅ <b>Grupo adicionado.</b>\n\nID: <code>${id}</code>`, [[{ text: "👥 Ver Grupos", callback_data: "menu:grupos" }], [{ text: "🏠 Painel", callback_data: "menu:inicio" }]]);
}

function parseButtons(text) { return text.split("\n").map(line => line.split("|").map(item => item.trim()).filter(Boolean).map(item => { const separator = item.indexOf(" - "); if (separator < 1) throw new Error(`Linha inválida: ${item}`); const label = item.slice(0, separator).trim(); const url = item.slice(separator + 3).trim(); if (!/^https?:\/\//i.test(url) && !/^tg:\/\//i.test(url)) throw new Error(`URL inválida no botão ${label}.`); return { text: label, url }; })); }
