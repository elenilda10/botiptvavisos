...
export async function handleDisparoCallback(env, callback) {
  ...
  if (data === "disparo:cancelar") {
    ...
    return enviarTelegram(env.TELEGRAM_TOKEN, "sendMessage", {
      chat_id: chatId, text: "❌ <b>Disparo cancelado.</b>\n\nNenhuma mensagem foi enviada.", parse_mode: "HTML",
  ...
  if (data === "disparo:confirmar") {
    ...
    const status = await enviarTelegram(env.TELEGRAM_TOKEN, "sendMessage", {
      chat_id: chatId, text: `🚀 <b>Iniciando disparo...</b>\n\n📡 Destinos: <b>${grupos.length}</b> grupos`, parse_mode: "HTML"
    });

    let sucessos = 0, erros = 0;
    const replyMarkup = state.buttons ? { inline_keyboard: state.buttons } : undefined;
    for (const grupoId of grupos) {
      try { await enviarConteudo(env, grupoId, state, replyMarkup); sucessos++; }
      catch (error) { erros++; console.error(`[DISPARO] Erro no grupo ${grupoId}:`, error); }
      await sleep(1500);
    }
...
export async function mostrarConfirmacao(env, chatId, state) {
  ...
  const confirmacao = await enviarTelegram(env.TELEGRAM_TOKEN, "sendMessage", {
    ...
    parse_mode: "HTML",
    reply_markup: { inline_keyboard: [[{ text: "✅ Confirmar Disparo", callback_data: "disparo:confirmar" }], [{ text: "❌ Cancelar", callback_data: "disparo:cancelar" }]] }
  });
...
async function apagarMensagem(env, chatId, messageId) {
  if (!messageId) return;
  try { await enviarTelegram(env.TELEGRAM_TOKEN, "deleteMessage", { chat_id: chatId, message_id: messageId }); }
  catch (error) { console.warn("[LIMPEZA] Não foi possível apagar mensagem:", error.message || error); }
}
...
async function enviarConteudo(env, chatId, state, replyMarkup) {
  ...
  switch (state.mediaType) {
    ...
    case "poll": {
      ...
      if (payload.type === "quiz" && Number.isInteger(poll.correct_option_id)) payload.correct_option_id = poll.correct_option_id;
      if (poll.explanation) payload.explanation = poll.explanation;
      return enviarTelegram(env.TELEGRAM_TOKEN, "sendPoll", payload);
    }
...
async function salvarHistorico(env, registro) {
  ...
  if (raw) try { historico = JSON.parse(raw); } catch { historico = []; }
